 ...
 * never duplicate it). The nav renders the logo/wordmark, a "Documentation"
 * link, the account menu (signed-in only), and the theme toggle — the
 * third-export header/footer restructure: "View on GitHub" moved OUT of the
 * header INTO the footer (now with the GitHub mark). Theme defaults to
 * `prefers-color-scheme`; the toggle sets `data-gc-theme` on <html> and
 * persists it in localStorage.
 *
 * `Layout` gains an optional `account`: absent/null → signed-out nav (Docs +
 * toggle); set → the AccountMenu renders before the toggle. The menu is a
 * pure-CSS <details>/<summary> disclosure — NO client JS, so the user-
 * controlled `@handle` and its initials never enter a script (they are
 * server-rendered and Hono-escaped into markup only). Caret rotation is an
 * app.css `[data-account-menu][open]` rule.
 */
...
/** Signed-in account descriptor threaded through the shell to the AccountMenu. */
export interface Account {
  handle: string;
}

export function Layout(props: { title: string; account?: Account | null; children?: Child }) {
  return (
    <>
      {raw('<!DOCTYPE html>')}
      <html lang="en">
        ...
        <body class="flex min-h-screen flex-col bg-paper font-display text-ink antialiased">
          <Nav account={props.account ?? null} />
...
function Nav(props: { account: Account | null }) {
  return (
    <div class="sticky top-0 z-20 border-b border-hair bg-nav backdrop-blur-[6px]">
      <div class="mx-auto flex h-14 max-w-[1120px] items-center justify-between px-4 sm:h-[58px] sm:px-7">
        ...
        <div class="flex items-center gap-[18px] sm:gap-[22px]">
          ...
          {props.account !== null ? <AccountMenu handle={props.account.handle} /> : null}
          <ThemeToggle />
        </div>
      </div>
    </div>
...
/**
 * Two-letter avatar initials from a handle (mirrors the mock's client-side
 * derivation, moved server-side): strip non-alphanumerics, lowercase, take
 * the first two chars, fall back to `gh`. Pure — safe to inline into markup.
 */
export function initials(handle: string): string {
  return (handle.replace(/[^a-z0-9]/gi, '').slice(0, 2) || 'gh').toLowerCase();
}

/** Sign-out (door-with-arrow) glyph from the account-menu mock. */
const LOGOUT_ICON =
  `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex:0 0 auto">` +
  `<path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/>` +
  `<polyline points="16 17 21 12 16 7"/>` +
  `<line x1="21" y1="12" x2="9" y2="12"/>` +
  `</svg>`;

/** GitHub's app-management screen — where a user revokes GitCert's access. */
const DISCONNECT_URL = 'https://github.com/settings/installations';

/**
 * Signed-in account control (account-menu.dc.html). A pure-CSS
 * <details>/<summary> disclosure — no client JS, so the user `handle` and its
 * initials appear ONLY in server-rendered, Hono-escaped markup and never in a
 * script (the strongest XSS posture; preferred over an inline toggle script
 * per the task). Trade-off vs the mock's JS: the panel closes on a second
 * summary click, not on outside-click/Escape. Caret rotation is the app.css
 * `[data-account-menu][open] [data-account-caret]` rule.
 *
 * Sign out is a real <form method="POST" action="/auth/logout"> (a button,
 * never a link — logout mutates session state). Disconnect is a subordinate,
 * muted external link to GitHub's own app-management screen; it is a distinct
 * action (revoke read access) from signing out (end the browser session).
 */
function AccountMenu(props: { handle: string }) {
  return (
    <details data-account-menu class="relative inline-flex">
      <summary
        aria-haspopup="menu"
        class="flex cursor-pointer list-none items-center gap-2 py-1 font-mono text-[13px] tracking-[0.2px] text-ink"
      >
      ...
      <div
        role="menu"
        class="absolute top-[calc(100%+13px)] right-0 z-[60] w-[276px] overflow-hidden rounded-[5px] border border-hair-strong bg-card text-left shadow-[0_10px_30px_rgba(0,0,0,0.16)]"
      >
        <div class="flex items-center gap-[11px] px-4 pt-[15px] pb-[13px]">
          <span class="inline-grid h-8 w-8 flex-none place-items-center rounded-full bg-avatar font-mono text-[12px] tracking-[0.4px] text-soft">
            {initials(props.handle)}
          </span>
          <div class="min-w-0">
            <div class="overflow-hidden font-mono text-[13px] font-semibold text-ellipsis whitespace-nowrap text-ink">
              @{props.handle}
            </div>
            <div class="mt-[2px] font-mono text-[11px] text-muted">signed in via GitHub</div>
          </div>
        </div>
        <div class="h-px bg-hair2" />
        <div class="px-[14px] py-[13px]">
          <form method="post" action="/auth/logout" class="m-0">
            <button
              type="submit"
              class="flex w-full cursor-pointer items-center justify-center gap-2 rounded-[3px] border border-hair-strong bg-card px-3 py-[9px] font-mono text-[13px] text-ink"
            >
              {raw(LOGOUT_ICON)}
              Sign out
            </button>
          </form>
          <div class="mt-[9px] font-mono text-[11px] leading-[1.55] text-muted">
            Ends this GitCert session only — your repo access stays connected.
          </div>
        </div>
        <div class="h-px bg-hair2" />
        <div class="px-[14px] pt-[13px] pb-[15px]">
          <a
            data-nav
            href={DISCONNECT_URL}
            target="_blank"
            rel="noopener"
            class="inline-flex items-center gap-[5px] font-mono text-[12px] text-muted"
          >
            Disconnect GitCert from GitHub <span class="text-[11px]">↗</span>
          </a>
          <div class="mt-[7px] font-mono text-[11px] leading-[1.55] text-faint">
            Removes GitCert&apos;s read access; your badges stop resolving.
          </div>
        </div>
      </div>
    </details>
